myApp.controller("ForumController", function($scope, $http, $location,$rootScope, $window) {
	...
	function fetchAllForums() {
		...
		$http.get("http://localhost:8085/CodeWarriaorsMiddleware/listForums").then(
				function(response) {
					console.log('Status text:' + response.statusText);
					$scope.forumData = response.data;
	...
	$scope.editForum = function(forumId) {
		...
		$http.get('http://localhost:8085/CodeWarriaorsMiddleware/getForum/' + forumId)
	...
	$scope.updateForum = function(forumId){
		...
		console.log(forumId);
		$http.put('http://localhost:8085/CodeWarriaorsMiddleware/updateForum/'+ forumId, $scope.forum)
		.then(fetchAllForums(), function(response){
	...
	$scope.deleteForum = function(forumId){
		...
		$http.delete('http://localhost:8085/CodeWarriaorsMiddleware/deleteForum/'+forumId)
		.then(fetchAllForums(), function(response){
